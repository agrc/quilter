...
try:
    ...
    if merge:
        ...
        if project:
            ...
            if proj_code[0].upper() == "ESRI": 
                ref.ImportFromESRI(int(proj_code[1]))
            elif proj_code[0].upper() == "EPSG": 
                ref.ImportFromEPSG(int(proj_code[1]))
            ref = None
    ...
    dl_folder = os.path.join(tdir, "dl")
    os.mkdir(dl_folder)
    progress = 0
    ...
    # Unzip to temp dir
    print("\nUnzipping files...")
    unzip_folder = os.path.join(tdir, "uz")
    ...
    # If we've gotten this far, we can go ahead and delete the downloaded zips
    delete = True
    ...
    # raster merging
        ...
        vrt_list = []
        vrt_path = os.path.join(tdir, final_name + str(os.getpid()) +
                                ".vrt")
        for dir_name, subdir_list, file_list in os.walk(extract_folder):
            for fname in file_list:
        ...
        if compress:
            co=["bigtiff=yes", "compress=lzw", "tiled=yes"]
        else:
            co=["tiled=yes"]
        ...
        if len(vrt_list) > 0:
            ...
            if 'COMPRESSION' in md and 'JPEG' in md["COMPRESSION"]:
                co = ["bigtiff=yes", "compress=jpeg", "tiled=yes"]
                gdal.SetConfigOption('COMPRESS_OVERVIEW', 'JPEG')
                if 'YCbCr' in md['COMPRESSION']:
                    co.append("photometric=ycbcr")
        ...
        if project:
            ...
            ds = gdal.Warp(raster_outpath, vrt_path, options=warp_opts)
            ds = None  # releases file handle

            print("\nBuilding overviews...")
            dso = gdal.Open(raster_outpath, gdal.GA_ReadOnly)
            dso.BuildOverviews("NEAREST", [2, 4, 8, 16], gdal_progress_callback)
            dso = None
        ...
        # otherwise, just translate vrt to tif
            ...
            trans_opts = gdal.TranslateOptions(format="GTiff", 
                                               creationOptions=co, 
                                               callback=gdal_progress_callback)
            ds = gdal.Translate(raster_outpath, vrt_path, options=trans_opts)
            ds=None
    ...
    if "proj_create_from_database" in e.args[0]:
        ...
        print("Whoops, something went wrong. Any finished downloads have been left in {}".format(tdir))
        delete = False
    print("\nPython error message:")
    print(e)
    # traceback.print_exc()
